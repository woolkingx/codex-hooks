...
| Data ontology | Schema follows data: event input, event rule, event output, plus system log/status/config/deploy/profile and authoring user requirements. No schema root without data ontology. |
...
| `schema/` | Official schema mirror plus system data schemas: log, status, config, deploy, profile, user-policy. Event rule schemas live with event owners. |
