...
## Scope
pre-tool-use channel: layer executor (input schema -> rule schema -> output schema).
...
- feature declarations are admitted by this event's `pre-tool-use.rule.schema.json`; feature value schemas stay with feature modules; no event metadata data root exists.
...
## Rules / Commands
- `@rules/rules.md`
- Tests: `node --test src/events/pre-tool-use/test.mjs`
